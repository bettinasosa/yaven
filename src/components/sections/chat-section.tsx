...
function CheckIcon() {
  return (
    <svg width="12" height="12" viewBox="0 0 12 12" fill="none" style={{ flexShrink: 0 }}>
      <circle cx="6" cy="6" r="5.5" fill="rgba(39,201,63,0.18)" stroke="rgba(39,201,63,0.50)" strokeWidth="0.75"/>
      <path d="M3.5 6L5.2 7.8L8.5 4.5" stroke="#27C93F" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  )
}

function TaskCard({ tag, trigger, steps, outcome }: typeof USE_CASES[number]) {
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "7px" }}>
          ...
          <span style={{
            ...
            fontSize: "12px", fontWeight: 700, color: T1,
          }}>Yaven</span>
        </div>
        <span style={{
      ...
      <div style={{ padding: "12px 14px 10px" }}>
        <p style={{
          ...
          color: T2, margin: 0, lineHeight: 1.45,
        }}>{trigger}</p>
      </div>
...
export function ChatSection() {
  ...
  useEffect(() => {
    ...
    const pop = (el: HTMLDivElement | null) => {
      if (!el) return
      gsap.to(el, { opacity: 1, y: 0, scale: 1, duration: 0.52, ease: "back.out(1.6)" })
    }
    ...
    const enterTrigger = ScrollTrigger.create({
      ...
      start: "top 72%",
      onEnter: () => {
        if (!revealed.current[0]) { revealed.current[0] = true; pop(cards[0]) }
      },
    })
